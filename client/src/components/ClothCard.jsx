...
const ClothCard = ({ cloth }) => {
  ...
  return (
    <div className="glass catalog-card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ height: '300px', backgroundColor: 'rgba(0,0,0,0.2)', position: 'relative' }}>
        <img 
          src={imageUrl} 
          alt={cloth.title} 
      ...
      <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Sizes: {cloth.size}</span>
        </div>
        <h3 style={{ fontSize: '1.25rem', marginTop: '0.5rem', marginBottom: '0.5rem', flexGrow: 1, letterSpacing: '-0.01em' }}>{cloth.title}</h3>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem' }}>
          <span style={{ fontSize: '1.25rem', fontWeight: 700 }}>{formatINR(cloth.pricePerDay)}<span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>/day</span></span>
          <Link
            to={`/cloth/${cloth._id}`}
