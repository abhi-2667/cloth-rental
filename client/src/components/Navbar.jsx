...
import { LogOut, User, LayoutDashboard, Lock, Home as HomeIcon, Shirt } from 'lucide-react';
          ...
          {user ? (
            <>
              ...
              <NavLink
                to="/admin"
                className={({ isActive }) => `nav-link-chip flex items-center gap-2 ${isActive ? 'nav-link-chip-active' : ''}`}
                title={user.role === 'admin' ? 'Admin dashboard' : 'Admin-only area'}
              >
                {user.role === 'admin' ? <LayoutDashboard size={18} /> : <Lock size={16} />}
                {user.role === 'admin' ? 'Admin' : 'Admin Only'}
              </NavLink>
