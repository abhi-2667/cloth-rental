...
const validateLoginLinkRequestPayload = (req, res, next) => {
  ...
  if (expectedRole && !['admin', 'user'].includes(expectedRole)) {
    return sendValidationError(res, 'expectedRole must be either admin or user');
  }

  return next();
};

const validateVerifyLoginLinkPayload = (req, res, next) => {
