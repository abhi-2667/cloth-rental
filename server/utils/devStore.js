...
const createMagicLinkSession = ({ jti, email, purpose, expectedRole, expiresAt }) => {
  const record = {
    _id: createId(),
    jti: String(jti),
  ...
  state.magicLinkSessions.push(record);
  return clone(record);
};

const consumeMagicLinkSession = ({ jti, email, purpose, expectedRole }) => {
  const normalizedEmail = String(email).toLowerCase();
  const session = state.magicLinkSessions.find((item) => item.jti === String(jti));
  ...
  if (session.email !== normalizedEmail || session.purpose !== purpose) {
    return { ok: false, reason: 'invalid' };
  }

  const normalizedExpectedRole = expectedRole || null;
  if (session.expectedRole !== normalizedExpectedRole) {
    return { ok: false, reason: 'invalid' };
  }
